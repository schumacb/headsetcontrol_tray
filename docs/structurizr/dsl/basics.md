 [Skip to main content](basics.md)   Link      Menu      Expand       (external link)    Document      Search       Copy       Copied
...
* Tags are comma separated (e.g. `Tag 1,Tag 2,Tag 3`) - see [Structurizr - Notation](https://docs.structurizr.com/ui/diagrams/notation) for details of how tags and styling works.

## Workspace rules

In addition, workspaces are subject to the following rules:

* Each view must have a unique “key” (this is generated for you if not specified; **automatically generated view keys are not guaranteed to be stable over time, and you will likely lose [manual layout information](https://docs.structurizr.com/ui/diagrams/manual-layout) when using them in conjunction with Structurizr Lite/on-premises/cloud**).
...
The `!const` keyword can be used to define a constant, which can be used with [string substitution](basics.md):
...
String substitution will take place on any text specified within a token using the `${NAME}` syntax, where `NAME` corresponds to a [string constant/variable](basics.md) or an environment variable. For example:
...
[Edit this page on GitHub.](https://github.com/structurizr/structurizr.github.io/tree/main/dsl/03-basics.md)