...
class ChatMixManager:
    ...
    def _set_stream_volume(self, stream_id: str, num_channels: int, target_volume: float):
        ...
        logger.debug(f"Setting volume for stream ID {stream_id} ({num_channels} channels) to {target_volume:.2f} with payload: {payload_json} (was verbose)") # Changed verbose to debug
