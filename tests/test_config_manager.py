...
class TestConfigManager(unittest.TestCase):
    ...
    @mock.patch.object(ConfigManager, '_load_json_file')
    @mock.patch.object(ConfigManager, '_save_json_file')
    def test_init_default_eq_curves_saved_if_empty(self, mock_save_json, mock_load_json):
        mock_load_json.side_effect = [{'some_setting': 'value'}, {}]
        cm = ConfigManager()
        self.mock_config_dir.mkdir.assert_called_once_with(parents=True, exist_ok=True)
        self.assertEqual(mock_load_json.call_count, 2)
        self.assertEqual(cm._custom_eq_curves, app_config.DEFAULT_EQ_CURVES)
        mock_save_json.assert_called_once_with(self.mock_eq_curves_file, app_config.DEFAULT_EQ_CURVES)

    @mock.patch('builtins.open', new_callable=mock.mock_open)
    @mock.patch('json.load')
    def test_load_json_file_success(self, mock_json_load, mock_open_file):
        mock_file_path = mock.MagicMock(spec=Path); mock_file_path.exists.return_value = True
        expected_data = {"key": "value"}; mock_json_load.return_value = expected_data
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {}; cm._custom_eq_curves = {}
    ...
    def test_load_json_file_does_not_exist(self):
        mock_file_path = mock.MagicMock(spec=Path); mock_file_path.exists.return_value = False
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {}; cm._custom_eq_curves = {}
            loaded_data = cm._load_json_file(mock_file_path)
        self.assertEqual(loaded_data, {})
    ...
    def test_save_json_file_success(self, mock_json_dump, mock_open_file):
        mock_file_path = mock.MagicMock(spec=Path); data_to_save = {"key": "value"}
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {}; cm._custom_eq_curves = {}
            cm._save_json_file(mock_file_path, data_to_save)
        mock_open_file.assert_called_once_with(mock_file_path, "w")
        mock_json_dump.assert_called_once_with(data_to_save, mock_open_file.return_value.__enter__.return_value, indent=4)

    @mock.patch('builtins.open', side_effect=IOError("Disk full"))
    @mock.patch('json.dump')
    def test_save_json_file_io_error(self, mock_json_dump, mock_open_file_raises_ioerror):
        mock_file_path = mock.MagicMock(spec=Path); data_to_save = {"key": "value"}
        with mock.patch.object(ConfigManager, '__init__', lambda x: None), \
             mock.patch('headsetcontrol_tray.config_manager.logger') as mock_logger:
            cm = ConfigManager(); cm._settings = {}; cm._custom_eq_curves = {}
    ...
    def test_save_json_file_os_error_on_dump(self, mock_json_dump_raises_oserror, mock_open_file):
        mock_file_path = mock.MagicMock(spec=Path); data_to_save = {"key": "value"}
        with mock.patch.object(ConfigManager, '__init__', lambda x: None), \
             mock.patch('headsetcontrol_tray.config_manager.logger') as mock_logger:
            cm = ConfigManager(); cm._settings = {}; cm._custom_eq_curves = {}
    ...
    def test_get_setting(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {"existing_key": "existing_value"}; cm._custom_eq_curves = {}
    ...
    def test_get_all_custom_eq_curves(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._custom_eq_curves = {"Curve1": [0]*10, "Curve2": [1]*10}
        self.assertEqual(cm.get_all_custom_eq_curves(), {"Curve1": [0]*10, "Curve2": [1]*10})

    def test_get_custom_eq_curve(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._custom_eq_curves = {"MyCurve": [1]*10}
    ...
    def test_save_custom_eq_curve_success(self, mock_save_json):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._custom_eq_curves = {"ExistingCurve": [0]*10}; cm._settings = {}
        new_curve_name = "NewCurve"; new_curve_values = [1]*10
        cm.save_custom_eq_curve(new_curve_name, new_curve_values)
        self.assertIn(new_curve_name, cm._custom_eq_curves)
        mock_save_json.assert_called_with(self.mock_eq_curves_file, cm._custom_eq_curves)
    ...
    def test_delete_custom_eq_curve(self, mock_save_json):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager()
            cm._custom_eq_curves = {"ToDelete": [0]*10, "ToKeep": [1]*10, app_config.DEFAULT_CUSTOM_EQ_CURVE_NAME: [0]*10}
        ...
        mock_save_json.assert_called_once_with(self.mock_eq_curves_file, cm._custom_eq_curves)
    ...
    def test_sidetone_level_shortcuts(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {}; cm._custom_eq_curves = {}
            with mock.patch.object(cm, 'get_setting') as mock_get, mock.patch.object(cm, 'set_setting') as mock_set:
                mock_get.return_value = 50
                self.assertEqual(cm.get_last_sidetone_level(), 50)
                mock_get.assert_called_once_with("sidetone_level", app_config.DEFAULT_SIDETONE_LEVEL)
    ...
    def test_eq_preset_id_shortcuts(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            ...
            with mock.patch.object(cm, 'get_setting') as mock_get, mock.patch.object(cm, 'set_setting') as mock_set:
                ...
                mock_get.assert_called_once_with("eq_preset_id", app_config.DEFAULT_EQ_PRESET_ID)
    ...
    def test_active_eq_type_shortcuts(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            ...
            with mock.patch.object(cm, 'get_setting') as mock_get, mock.patch.object(cm, 'set_setting') as mock_set:
                ...
                mock_get.assert_called_once_with("active_eq_type", self.CM_DEFAULT_ACTIVE_EQ_TYPE)
    ...
    def test_get_last_custom_eq_curve_name_fallbacks(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            ...
            cm._custom_eq_curves = {"MyFavCurve": [1]*10, app_config.DEFAULT_CUSTOM_EQ_CURVE_NAME: [0]*10}
            self.assertEqual(cm.get_last_custom_eq_curve_name(), "MyFavCurve")

            cm._settings = {"last_custom_eq_curve_name": "NonExistent"}
            cm._custom_eq_curves = {app_config.DEFAULT_CUSTOM_EQ_CURVE_NAME: [0]*10, "AnotherCurve": [2]*10}
    ...
    def test_set_last_custom_eq_curve_name(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            ...
            with mock.patch.object(cm, 'set_setting') as mock_set:
                ...
                mock_set.assert_any_call("last_custom_eq_curve_name", "MyNewCustomCurve")
    ...
    def test_get_setting_for_chatmix_enabled(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {};
            self.assertEqual(cm.get_setting("chatmix_enabled", self.CM_DEFAULT_CHATMIX_ENABLED), True)
            cm._settings = {"chatmix_enabled": False}
            self.assertEqual(cm.get_setting("chatmix_enabled", self.CM_DEFAULT_CHATMIX_ENABLED), False)

    def test_get_setting_for_auto_mute_mic_enabled(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {};
            self.assertEqual(cm.get_setting("auto_mute_mic_enabled", self.CM_DEFAULT_AUTO_MUTE_MIC_ENABLED), False)
            cm._settings = {"auto_mute_mic_enabled": True}
            self.assertEqual(cm.get_setting("auto_mute_mic_enabled", self.CM_DEFAULT_AUTO_MUTE_MIC_ENABLED), True)

    def test_get_setting_for_run_on_startup_enabled(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {};
            self.assertEqual(cm.get_setting("run_on_startup_enabled", self.CM_DEFAULT_RUN_ON_STARTUP_ENABLED), True)
            cm._settings = {"run_on_startup_enabled": False}
            self.assertEqual(cm.get_setting("run_on_startup_enabled", self.CM_DEFAULT_RUN_ON_STARTUP_ENABLED), False)

    def test_get_setting_for_minimize_to_tray_enabled(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {};
            self.assertEqual(cm.get_setting("minimize_to_tray_enabled", self.CM_DEFAULT_MINIMIZE_TO_TRAY_ENABLED), True)
            cm._settings = {"minimize_to_tray_enabled": False}
            self.assertEqual(cm.get_setting("minimize_to_tray_enabled", self.CM_DEFAULT_MINIMIZE_TO_TRAY_ENABLED), False)

    def test_get_setting_for_check_for_updates_enabled(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {};
            self.assertEqual(cm.get_setting("check_for_updates_enabled", self.CM_DEFAULT_CHECK_FOR_UPDATES_ENABLED), True)
            cm._settings = {"check_for_updates_enabled": False}
            self.assertEqual(cm.get_setting("check_for_updates_enabled", self.CM_DEFAULT_CHECK_FOR_UPDATES_ENABLED), False)

    def test_get_setting_for_include_prereleases_enabled(self):
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {};
            self.assertEqual(cm.get_setting("include_prereleases_enabled", self.CM_DEFAULT_INCLUDE_PRERELEASES_ENABLED), False)
            cm._settings = {"include_prereleases_enabled": True}
            self.assertEqual(cm.get_setting("include_prereleases_enabled", self.CM_DEFAULT_INCLUDE_PRERELEASES_ENABLED), True)

    def test_get_setting_for_last_selected_device_serial(self): # Covers get_last_selected_device_serial
        with mock.patch.object(ConfigManager, '__init__', lambda x: None):
            cm = ConfigManager(); cm._settings = {};
