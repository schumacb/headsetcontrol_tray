...
def create_status_response_data(
    ...
    data = bytearray([0] * app_config.HID_INPUT_REPORT_LENGTH_STATUS) # Use bytearray for mutability
    ...
    if app_config.HID_RES_STATUS_BATTERY_STATUS_BYTE < len(data):
        ...
        raise IndexError(f"HID_RES_STATUS_BATTERY_STATUS_BYTE index {app_config.HID_RES_STATUS_BATTERY_STATUS_BYTE} out of bounds for data length {len(data)}")

    if app_config.HID_RES_STATUS_BATTERY_LEVEL_BYTE < len(data):
        data[app_config.HID_RES_STATUS_BATTERY_LEVEL_BYTE] = level_byte_val
    else:
        raise IndexError(f"HID_RES_STATUS_BATTERY_LEVEL_BYTE index {app_config.HID_RES_STATUS_BATTERY_LEVEL_BYTE} out of bounds for data length {len(data)}")

    if app_config.HID_RES_STATUS_CHATMIX_GAME_BYTE < len(data):
        data[app_config.HID_RES_STATUS_CHATMIX_GAME_BYTE] = game_byte_val
    else:
        raise IndexError(f"HID_RES_STATUS_CHATMIX_GAME_BYTE index {app_config.HID_RES_STATUS_CHATMIX_GAME_BYTE} out of bounds for data length {len(data)}")

    if app_config.HID_RES_STATUS_CHATMIX_CHAT_BYTE < len(data):
        data[app_config.HID_RES_STATUS_CHATMIX_CHAT_BYTE] = chat_byte_val
    else:
        raise IndexError(f"HID_RES_STATUS_CHATMIX_CHAT_BYTE index {app_config.HID_RES_STATUS_CHATMIX_CHAT_BYTE} out of bounds for data length {len(data)}")
...
class TestHeadsetStatusParser(unittest.TestCase):
    ...
    def test_parse_status_report_online_charging_full_battery_mid_chatmix(self, mock_logger):
        # Status: 0x01 (Charging), Level: 0x04 (100%), Game: 50, Chat: 50 (Balanced -> UI 64)
        response_data = create_status_response_data(status_byte_val=0x01, level_byte_val=0x04, game_byte_val=50, chat_byte_val=50)
    ...
    def test_parse_status_report_offline(self, mock_logger):
        # Status: 0x00 (Offline)
        response_data = create_status_response_data(status_byte_val=0x00, level_byte_val=0x02, game_byte_val=50, chat_byte_val=50)
    ...
    def test_parse_status_report_various_battery_levels(self, mock_logger):
    ...
    def test_parse_status_report_various_chatmix_values(self, mock_logger):
        # status_byte_val=0x02 (Online, not charging), level_byte_val=0x02 (50%)
        chatmix_tests = [
            # (game_raw, chat_raw, expected_ui_value)
            (100, 0, 0), (0, 100, 128), (50, 50, 64), (0, 0, 64),
            (75, 25, 32), (25, 75, 96)
        ]
    ...
    def test_determine_headset_online_status_short_data(self, mock_logger):
        ...
        mock_logger.warning.assert_called_with(f"_determine_headset_online_status: Response data too short. Expected > {app_config.HID_RES_STATUS_BATTERY_STATUS_BYTE} bytes, got {len(short_data)}")
...
class TestHeadsetCommandEncoder(unittest.TestCase):
    ...
    def test_encode_set_sidetone(self, mock_logger):
        # (level_ui, expected_hw_byte)
        sidetone_map = {0: 0x00, 25: 0x00, 26: 0x01, 50: 0x01, 51: 0x02, 75: 0x02, 76: 0x03, 128: 0x03}
        for ui_level, hw_byte in sidetone_map.items():
            with self.subTest(ui_level=ui_level):
                expected_payload = list(app_config.HID_CMD_SET_SIDETONE_PREFIX) + [hw_byte]
                encoded = self.encoder.encode_set_sidetone(ui_level)
                self.assertEqual(encoded, expected_payload)

    def test_encode_set_inactive_timeout(self, mock_logger):
        ...
        for minutes_in, minutes_byte in timeout_map.items():
            with self.subTest(minutes_in=minutes_in):
                expected_payload = list(app_config.HID_CMD_SET_INACTIVE_TIME_PREFIX) + [minutes_byte]
                encoded = self.encoder.encode_set_inactive_timeout(minutes_in)
                self.assertEqual(encoded, expected_payload)

    def test_encode_set_eq_values_valid(self, mock_logger):
    ...
    def test_encode_set_eq_values_invalid_band_count(self, mock_logger):
        eq_floats_short = [0.0] * 9
        encoded = self.encoder.encode_set_eq_values(eq_floats_short)
        self.assertIsNone(encoded)
        mock_logger.error.assert_called_with("encode_set_eq_values: Invalid number of EQ bands. Expected 10, got 9.")
    ...
    def test_encode_set_eq_preset_id_valid(self, mock_logger):
        # Assume preset ID 0 exists and has 10 float values.
        preset_id_to_test = 0
        if preset_id_to_test not in app_config.ARCTIS_NOVA_7_HW_PRESETS:
            self.skipTest(f"Preset ID {preset_id_to_test} not in app_config.ARCTIS_NOVA_7_HW_PRESETS for testing.")
    ...
    def test_encode_set_eq_preset_id_invalid_id(self, mock_logger):
        ...
        self.assertIsNone(encoded)
        mock_logger.error.assert_called_with(f"encode_set_eq_preset_id: Invalid preset ID: {invalid_id}. Not in ARCTIS_NOVA_7_HW_PRESETS.")

    @patch.dict(app_config.ARCTIS_NOVA_7_HW_PRESETS, {
        0: {"name": "TestPreset", "values": [1.0]*5} # Malformed: 5 bands instead of 10
    })
    def test_encode_set_eq_preset_id_malformed_preset_data_band_count(self, mock_logger):
        encoded = self.encoder.encode_set_eq_preset_id(0)
        self.assertIsNone(encoded)
        mock_logger.error.assert_any_call("encode_set_eq_preset_id: Malformed preset data for ID 0. Expected 10 bands, got 5.")

    @patch.dict(app_config.ARCTIS_NOVA_7_HW_PRESETS, {
        0: {"name": "TestPreset", "values": "not_a_list"} # Malformed: values not a list
    })
    def test_encode_set_eq_preset_id_malformed_preset_data_values_type(self, mock_logger):
        encoded = self.encoder.encode_set_eq_preset_id(0)
        self.assertIsNone(encoded)
        mock_logger.error.assert_any_call("encode_set_eq_preset_id: Preset data 'values' for ID 0 is not a list of numbers.")
