...
@pytest.mark.skip(reason="Known to fail: runpy does not work well with patching __main__.main, and it exposes QApplication init crash.")
...
@pytest.mark.skip(reason="Known to fail: runpy does not work well with patching for __main__ execution, and it exposes QApplication init crash.")
@mock.patch('headsetcontrol_tray.__main__.SteelSeriesTrayApp') # Patch SteelSeriesTrayApp
...
def test_main_block_execution_revised(mock_signal, mock_sys_exit, MockSteelSeriesTrayApp):
