...
# Import HIDConnectionManager for mocking - REMOVED to break circular dependency for tests
...
def linux_impl_fixture():  # mocker removed
    ...
    with (
        patch("headsetcontrol_tray.os_layer.linux.UDEVManager", autospec=True) as mock_udev_manager_class, # Reinstated autospec for UDEVManager
        patch("headsetcontrol_tray.os_layer.linux.HIDConnectionManager", autospec=True) as mock_hid_manager_class,
    ):
        mock_udev_manager_instance = mock_udev_manager_class.return_value
...
def test_linux_impl_get_hid_manager(linux_impl_fixture):  # mocker removed
    ...
    assert hasattr(hid_manager, "assert_called_once")  # Check for mock-like behavior
...
def test_linux_impl_perform_device_setup_prepare_fails(linux_impl_fixture):
    linux_impl_fixture._udev_manager.create_rules_interactive.return_value = False  # Corrected method name
...
def test_linux_impl_perform_device_setup_pkexec_success(linux_impl_fixture):  # mocker removed
    linux_impl_fixture._udev_manager.create_rules_interactive.return_value = True  # Corrected method name
    ...
    with patch.object(
        linux_impl_fixture, "_execute_udev_helper_script", return_value=mock_completed_process,
    ) as mock_exec_script:
        success, proc_result, error = linux_impl_fixture.perform_device_setup()
...
def test_linux_impl_perform_device_setup_pkexec_script_failures(
    linux_impl_fixture, pkexec_returncode, expected_success,
):  # mocker removed
    linux_impl_fixture._udev_manager.create_rules_interactive.return_value = True  # Corrected method name
    mock_udev_details = {"temp_file_path": "/tmp/temp.rules", "final_file_path": "/etc/final.rules"}
    linux_impl_fixture._udev_manager.get_last_udev_setup_details.return_value = mock_udev_details
...
def test_linux_impl_perform_device_setup_exceptions(
    linux_impl_fixture, exception_to_raise, expected_exception_type,
):  # mocker removed
    linux_impl_fixture._udev_manager.create_rules_interactive.return_value = True  # Corrected method name
    mock_udev_details = {"temp_file_path": "/tmp/temp.rules", "final_file_path": "/etc/final.rules"}
    linux_impl_fixture._udev_manager.get_last_udev_setup_details.return_value = mock_udev_details
    ...
    with patch.object(
        linux_impl_fixture, "_execute_udev_helper_script", side_effect=exception_to_raise,
    ) as mock_exec_script:
        success, proc_result, error = linux_impl_fixture.perform_device_setup()
    ...
    def test_linux_impl_internal_execute_helper_script_calls_subprocess_run(linux_impl_fixture):
        ...
        with (
            ...
            mock_script_path_obj = MagicMock(spec=Path, __str__=MagicMock(return_value=expected_script_path_str))
            mock_script_path_obj.is_file.return_value = True
            # Crucially, mock the __str__ method is now done in the constructor
            ...
            def path_side_effect(value):
                ...
                generic_path_mock = MagicMock(spec=Path, __str__=MagicMock(return_value=str(value) if value else "."))
                generic_path_mock.is_file.return_value = True  # Default to file existing
                # __str__ mock is now done in the constructor
