...
def create_mock_device_info(pid, interface_number=-1, usage_page=0, usage=0, path_suffix="1"):
    return {
        ...
        "usage": usage,
        "path": f"path_vid_{app_config.STEELSERIES_VID:04x}_pid_{pid:04x}_{path_suffix}".encode('utf-8'),
...
class TestHIDConnectionManagerDiscovery(unittest.TestCase):
    ...
    def test_find_potential_hid_devices_success(self, mock_logger, mock_hid_enumerate):
        ...
        mock_hid_enumerate.return_value = [
            ...
            create_mock_device_info(mock_dev_other_vid_pid), # Belongs to SteelSeries VID, but not target PID
            {"vendor_id": 0x1234, "product_id": 0x5678, "path": b"other_path"} # Different VID
        ]
    ...
    @patch('headsetcontrol_tray.hid_manager.hid.enumerate')
    @patch('headsetcontrol_tray.hid_manager.logger')
    def test_find_potential_hid_devices_enumeration_error(self, mock_logger, mock_hid_enumerate):
        mock_hid_enumerate.side_effect = Exception("Enumeration failed")
        devices = self.manager._find_potential_hid_devices()
        self.assertEqual(len(devices), 0)
        mock_logger.error.assert_called_with("Error enumerating HID devices: Enumeration failed")

    @patch('headsetcontrol_tray.hid_manager.hid.enumerate')
    @patch('headsetcontrol_tray.hid_manager.logger')
    def test_find_potential_hid_devices_no_matches(self, mock_logger, mock_hid_enumerate):
        mock_hid_enumerate.return_value = [
...
class TestHIDConnectionManagerSorting(unittest.TestCase):
    ...
    def test_sort_hid_devices(self):
        ...
        dev_c = create_mock_device_info(app_config.TARGET_PIDS[0], interface_number=3, path_suffix="dev_c")
        # Usage page 0xFFC0 (priority: 1)
        dev_d = create_mock_device_info(app_config.TARGET_PIDS[0], usage_page=0xFFC0, path_suffix="dev_d")
        # Default (priority: 2)
        dev_e = create_mock_device_info(app_config.TARGET_PIDS[0], interface_number=1, path_suffix="dev_e")
...
@patch.object(HIDConnectionManager, '_find_potential_hid_devices') # Mock the method within the class
class TestHIDConnectionManagerConnection(unittest.TestCase):
    def setUp(self):
        self.manager = HIDConnectionManager()

    def test_connect_device_success(self, mock_find_devices, mock_hid_device_constructor):
        mock_device_info = create_mock_device_info(app_config.TARGET_PIDS[0], interface_number=app_config.HID_REPORT_INTERFACE)
        mock_find_devices.return_value = [mock_device_info] # Already sorted by virtue of being only one
        ...
        mock_hid_device_constructor.assert_called_once_with(path=mock_device_info["path"])

    def test_connect_device_no_devices_found(self, mock_find_devices, mock_hid_device_constructor):
    ...
    @patch('headsetcontrol_tray.hid_manager.logger')
    def test_connect_device_open_fails_for_all(self, mock_logger, mock_find_devices, mock_hid_device_constructor):
        mock_dev_info1 = create_mock_device_info(app_config.TARGET_PIDS[0], path_suffix="fail1")
        mock_dev_info2 = create_mock_device_info(app_config.TARGET_PIDS[0], path_suffix="fail2")
        mock_find_devices.return_value = [mock_dev_info1, mock_dev_info2] # Assume already sorted
        ...
        self.assertFalse(result)
        self.assertIsNone(self.manager.hid_device)
        self.assertEqual(mock_hid_device_constructor.call_count, 2) # Tried both devices
        mock_logger.warning.assert_called()

    @patch.object(HIDConnectionManager, '_connect_device')
    def test_ensure_connection_already_connected(self, mock_internal_connect_device, mock_find_devices_unused, mock_hid_device_constructor_unused):
    ...
    @patch.object(HIDConnectionManager, '_connect_device')
    def test_ensure_connection_needs_connection(self, mock_internal_connect_device, mock_find_devices_unused, mock_hid_device_constructor_unused):
