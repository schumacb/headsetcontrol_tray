    ...
    def _sort_hid_devices(self, devices: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        def sort_key(d_info: dict[str, Any]) -> int:
            ...
            if (
                ...
                logger.debug(
                    "  SortKey: Prioritizing interface 0 for PID 0x%04x (-1)",
                    d_info.get("product_id"),
                )
                return -1
            if (
                d_info.get("interface_number") == 3
            ):  # Common interface for some SteelSeries headsets
                logger.debug(
                    "  SortKey: Prioritizing interface 3 (generic) for PID 0x%04x (0)",
                    d_info.get("product_id"),
                )
                return 0
            if d_info.get("usage_page") == 0xFFC0:  # Common SteelSeries usage page
                logger.debug(
                    "  SortKey: Prioritizing usage page 0xFFC0 (generic) for PID 0x%04x (1)",
                    d_info.get("product_id"),
                )
                return 1
            logger.debug(
        ...
        logger.debug(
            "Found %s potential devices to try, sorted by priority.", len(devices),
        )
    ...
    def _connect_device(self) -> bool:
        ...
        for dev_info_to_try in sorted_devices:
            ...
            try:
                ...
                logger.warning(
                    "    Failed to open HID device path %s: %s", path_str, e_open,
                )
                if h_temp:
                    try:
                        h_temp.close()
    ...
    def close(self) -> None:
        ...
        if self.hid_device:
            ...
            if self.selected_device_info and isinstance(
                self.selected_device_info.get("path"), bytes,
            ):
            ...
            try:
                ...
                logger.warning(
                    "Exception while closing HID device %s: %s", device_path, e,
                )
            finally:
                self.hid_device = None
                self.selected_device_info = None
